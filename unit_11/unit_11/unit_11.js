...
function f11() {
    ...
    document.querySelector('.out-11').textContent = d11.indexOf(num);
...
// Task 18
// Напишите функцию f18, которая получает значение из i-18 и проверяет есть ли такое значение в массиве d18 c 
// использованием метода include. Результат применения метода - выводится в .out-18.
// Во всех задачах где говорим второй, третий и т.д. имеем ввиду индекс
...
function f18() {
    ...
    document.querySelector('.out-18').textContent = d18.includes(num);
}
...
function f19() {
    ...
    for (let i = 0; i < d19.length; i++) {
        if (d19[i].length > maxString) {
             maxString = d19[i].length;
             elem = d19[i];
        }
    }
    document.querySelector('.out-19').textContent = elem;
}
