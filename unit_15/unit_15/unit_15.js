...
const f7 = () => {
    ...
    let arr = value.split('');
    let s7 = new Set(arr);
    if (arr.length == s7.size && s7.size > 6) {
...
const f9 = our_set => {
    let out = Array.from(our_set).join(' ');
    return out;
}
...
// перебирая набор поэлементам, найти сколько раз каждый символ встречается в исходном массиве. Результат - в виде объекта типа { символ : количество, символ : количество } вывести в консоль и возвратить.
