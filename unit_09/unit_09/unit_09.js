...
//  Task 2
// По нажатию на кнопку .b-2 функция f2, которая присваивает блоку 
//.out-2 класс .bg-orange.
...
function f5() {
    let result = document.querySelector('.out-4').classList.contains('bg-orange'); 
...
function f6() {
    let num ='';
    let list = document.querySelectorAll('.p-6');
    // for(let i = 0; i < list.length; i++){
    //     num = +num + 1 ;
    // }
    document.querySelector('.out-6').textContent = list.length;
}
...
function f12() {
    ...
    document.querySelector('.out-12').prepend(div);
}
...
function f13() {
    ...
    document.querySelector('.out-13').prepend(div);
    div.onclick =  f13_1;
}
...
let attribute = p.getAttribute('data-b');
document.querySelector('.out-18').textContent = attribute;
}
...
function f19() {
    let out = '';
    let list = document.querySelectorAll('.p-19');
    for(let i = 0; i < list.length; i++){
        out += list[i].getAttribute('data-b') + " ";
    }
