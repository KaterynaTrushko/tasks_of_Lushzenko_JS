...
// Task 2
//При нажатии .b-2 выполняете функцию f2. Функция перебирает массив a2  c помощью цикла for (let i. Выведите на страницу 
// в .out-2 формате индекс+пробел+значение+пробел.
...
function f3() {
    let items = document.querySelectorAll('div.out-3');
...
function f4() {
    let items = document.querySelectorAll('div.out-4');
    for (let item of items) {
        item.textContent = 4;
    }
}
...
// c помощью document.querySelectorAll, затем c помощью for of перебрать полученную коллекцию элементов .out-15 записать внутрь них число 15 ( затерев содержимое).


function f15() {
    let lists = document.querySelectorAll('.out-15');
    for (let el of lists) {
        el.textContent = 15;
    }
}
